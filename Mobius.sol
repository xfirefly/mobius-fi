...
contract Mobius is  ReentrancyGuard {
  ...
  constructor(
    ...
    public
  {
    require(_governance != ZERO_ADDRESS, "Mobius: governance address cannot be 0x0.");
    require(_sentinel != ZERO_ADDRESS, "Mobius: sentinel address cannot be 0x0.");
  ...
  function setPendingGovernance(address _pendingGovernance) external onlyGov {
    require(_pendingGovernance != ZERO_ADDRESS, "Mobius: governance address cannot be 0x0.");
  ...
  function setTransmuter(address _transmuter) external onlyGov {
    ...
    require(_transmuter != ZERO_ADDRESS, "Mobius: transmuter address cannot be 0x0.");
  ...
  function setRewards(address _rewards) external onlyGov {
    ...
    // transfers to the address because of `safeTransfer` checks.
    require(_rewards != ZERO_ADDRESS, "Mobius: rewards address cannot be 0x0.");
  ...
  function setHarvestFee(uint256 _harvestFee) external onlyGov {
    ...
    require(_harvestFee <= PERCENT_RESOLUTION, "Mobius: harvest fee above maximum.");
  ...
  function setCollateralizationLimit(uint256 _limit) external onlyGov {
    ...
    require(_limit >= MINIMUM_COLLATERALIZATION_LIMIT, "Mobius: collateralization limit below minimum.");
    require(_limit <= MAXIMUM_COLLATERALIZATION_LIMIT, "Mobius: collateralization limit above maximum.");
  ...
  function initialize(IVaultAdapter _adapter) external onlyGov {
    ...
    require(!initialized, "Mobius: already initialized");

    require(transmuter != ZERO_ADDRESS, "Mobius: cannot initialize transmuter address to 0x0");
    require(rewards != ZERO_ADDRESS, "Mobius: cannot initialize rewards address to 0x0");
  ...
  function mint(uint256 _amount) external nonReentrant noContractAllowed onLinkCheck expectInitialized {
    ...
    if (_totalCredit < _amount) {
      ...
      _cdp.checkHealth(_ctx, "Mobius: Loan-to-value ratio breached");
    } else {
  ...
  modifier expectInitialized() {
    require(initialized, "Mobius: not initialized.");
    _;
  }
  ...
  ///
  modifier onlyGov() {
    require(msg.sender == governance, "Mobius: only governance.");
    _;
  }
  ...
  function _updateActiveVault(IVaultAdapter _adapter) internal {
    require(_adapter != IVaultAdapter(ZERO_ADDRESS), "Mobius: active vault address cannot be 0x0.");
    require(_adapter.token() == token, "Mobius: token mismatch.");
  ...
  function _recallFunds(uint256 _vaultId, uint256 _amount) internal returns (uint256, uint256) {
    require(emergencyExit || msg.sender == governance || _vaultId != _vaults.lastIndex(), "Mobius: not an emergency, not governance, and user does not have permission to recall funds from active vault");
