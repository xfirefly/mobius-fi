...
/// @title AlToken
///
/// @dev This is the contract for the Mobius utillity token usd.
///
contract AlToken is AccessControl, ERC20("Mobius USD", "mUSD") {
  ...
  modifier onlyWhitelisted() {
    require(whiteList[msg.sender], "mUSD: Mobius is not whitelisted");
    _;
  }
  ...
  function mint(address _recipient, uint256 _amount) external onlyWhitelisted {
    require(!blacklist[msg.sender], "mUSD: Mobius is blacklisted.");
    uint256 _total = _amount.add(hasMinted[msg.sender]);
    require(_total <= ceiling[msg.sender],"mUSD: Mobius's ceiling was breached.");
