...
contract Transmuter is Context {
    ...
    /// @dev mobius addresses whitelisted
    ...
    event TokensTransmuted(
        address indexed account,
        uint256 requestedAmount
    );

    event TokensTransmutedAndClaimed(
        address indexed account
    );
    ...
    function transmute() public runPhasedDistribution() updateAccount(msg.sender) {
        ...
        emit TokensTransmuted(msg.sender, pendingz );
    }
    ...
    function transmuteAndClaim() public {
        ...
        emit TokensTransmutedAndClaimed(msg.sender );
    }
    ...
    ///
    /// This function is meant to be called by the Mobius contract for when it is sending yield to the transmuter. 
